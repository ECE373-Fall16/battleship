...
public class ShipSetup extends GameEngine{
	...
	static String hitme = "me-xxx";
	static String hitmeenemy = "enemy-xxx";


	static ArrayList<Integer> ListShot = new ArrayList<>();
	static ArrayList<Integer> ListEnemyShot = new ArrayList<>();
	
	
	public static int getRealHit(String hit){
		hit = hit.replaceAll("[^0-9]","");
		if((hit.length() > 2) || (hit.length() < 1)){
	...
	public static String removeShip(int location){
		...
		if(ran_through_me == false){
			if(intList.contains(new Integer(location))){
	...
	public static String removeEnemyShip(int location){
		...
		if(ran_through_enemy == false){
			if(intListEnemy.contains(new Integer(location))){
				System.out.println("the location recieved in shipsetup is "+ location);
